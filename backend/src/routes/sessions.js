...
// POST /api/sessions
// Body: { quizId }
// Creates a new live session and returns joinCode + hostCode
...
sessionsRouter.post('/:joinCode/answer', async (req, res, next) => {
  try {
    ...
    session.answers[qIndex][playerId] = optionIndex;
...
// Body: { hostCode }
// Moves the game forward when the current phase time is finished.
sessionsRouter.post('/:joinCode/tick', async (req, res, next) => {
  try {
    ...
    if (session.hostCode !== String(hostCode).trim().toUpperCase()) {
